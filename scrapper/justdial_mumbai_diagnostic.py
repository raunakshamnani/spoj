...
# Total : for i in range ( 1 , 21 ) :
# Done : 
# for i in range ( 1 , 2 ) :

for i in range (20,21):
	link = "http://www.justdial.com/Mumbai/Diagnostic-Centres/ct-39530/page-" + str(i)
	r  = requests.get(link)
	data = r.text
		...
		for new_link in medical.find_all('a'):
			...
			for continfo in soup2.find_all('aside' , class_="continfo"):
				...
				for address in continfo.find_all('span' , class_="jaddt"):
					...
					address_list = final_address.split(',')
					print (address_list)
					# print ("Address List : ")
					part_count = 0
					area_count = 0
					...
					for i in range(0,len(address_list)):
						if ( ( address_list[i][-6:].isdigit() ) and ( len(address_list[i]) > 8 ) ):
							print ("City : " + address_list[i][:-8])
							shop_city.append(address_list[i][:-8])
							print ("PIN Code : " + address_list[i][-6:])
							shop_pin.append(address_list[i][-6:])
							# print ("I : " + str(i))
							break

					print ("Area : " + address_list[i-1])
					shop_area.append(address_list[i-1])

					print ("Road : " + address_list[i-2])
					shop_road.append(address_list[i-2])
...
final = zip (shop_name,shop_number,shop_address,shop_road,shop_area,shop_city,shop_pin,shop_rating,shop_year,shop_x_coordinate,shop_y_coordinate,shop_monday,shop_tuesday,shop_wednesday,shop_thursday,shop_friday,shop_saturday,shop_sunday)

with open('justdial(20).csv' , 'w') as c:
	writer = csv.writer(c , lineterminator='\n')
	writer.writerow(['Name','Number','Address','Road','Area','City','PIN','Rating','Year','X','Y','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'])
	for route in final:
		writer.writerow([route[0],route[1],route[2],route[3],route[4],route[5],route[6],route[7],route[8],route[9],route[10],route[11],route[12],route[13],route[14],route[15],route[16],route[17]])
c.close()
